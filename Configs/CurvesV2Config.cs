...
namespace NinjaTrader.NinjaScript.Strategies.OrganizedStrategy
{
    ...
    public class CurvesV2Config
    {
        ...
        // WebSocket settings
        [Display(Name = "Use WebSocket Only", Description = "Use WebSocket exclusively for all communications", Order = 6, GroupName = "WebSocket")]
        public bool UseWebSocketOnly { get; set; } = true;
        
        [Display(Name = "WebSocket Backtest Timeout (ms)", Description = "Timeout for backtest WebSocket requests in milliseconds", Order = 7, GroupName = "WebSocket")]
        [Range(1, 60000)]
        public int WebSocketBacktestTimeoutMs { get; set; } = 250;
        
        [Display(Name = "WebSocket Reconnect Attempts", Description = "Number of reconnection attempts for WebSocket", Order = 8, GroupName = "WebSocket")]
        [Range(1, 10)]
        public int WebSocketReconnectAttempts { get; set; } = 3;

        // Signal thresholds
        [Display(Name = "Bull Signal Threshold", Description = "Minimum value for bull signal", Order = 9, GroupName = "Signals")]
        [Range(0, 100)]
        public int BullSignalThreshold { get; set; } = 70;

        [Display(Name = "Bear Signal Threshold", Description = "Minimum value for bear signal", Order = 10, GroupName = "Signals")]
        [Range(0, 100)]
        public int BearSignalThreshold { get; set; } = 70;

        [Display(Name = "Pattern Confidence Threshold", Description = "Minimum confidence for pattern matching", Order = 11, GroupName = "Signals")]
        [Range(0, 1)]
        public double PatternConfidenceThreshold { get; set; } = 0.7;

        // Rate limiting
        [Display(Name = "Bar Data Interval (ms)", Description = "Minimum time between bar data updates in milliseconds", Order = 12, GroupName = "Rate Limiting")]
        [Range(100, 10000)]
        public int BarDataIntervalMs { get; set; } = 1000;

        [Display(Name = "Signal Check Interval (ms)", Description = "Minimum time between signal checks in milliseconds", Order = 13, GroupName = "Rate Limiting")]
        [Range(100, 10000)]
        public int SignalCheckIntervalMs { get; set; } = 1000;
        
        [Display(Name = "Signal Poll Interval (ms)", Description = "Interval for automatic signal polling in milliseconds", Order = 14, GroupName = "Rate Limiting")]
        [Range(1000, 10000)]
        public int SignalPollIntervalMs { get; set; } = 3000;

        // Synchronous mode settings
        [Display(Name = "Enable Sync Mode", Description = "Use synchronous processing for NinjaTrader integration", Order = 15, GroupName = "Processing")]
        public bool? EnableSyncMode { get; set; } = null;

        // Logging
        [Display(Name = "Enable Detailed Logging", Description = "Enable detailed logging of API calls", Order = 16, GroupName = "Logging")]
        public bool EnableDetailedLogging { get; set; } = false;

        // Pattern auction settings
        [Display(Name = "Enable Pattern Auction", Description = "Enable pattern auction system", Order = 17, GroupName = "Pattern Auction")]
        public bool EnablePatternAuction { get; set; } = true;

        [Display(Name = "Performance Decay Rate", Description = "How quickly pattern performance decays (0-1)", Order = 18, GroupName = "Pattern Auction")]
        [Range(0, 1)]
        public double PerformanceDecayRate { get; set; } = 0.1;

        [Display(Name = "Minimum Pattern Trades", Description = "Minimum number of trades before pattern performance is considered reliable", Order = 19, GroupName = "Pattern Auction")]
        [Range(1, 100)]
